...
def fever_accuracy(preds,labels,guid1_batch,guid2_batch):
    refined_preds=list()
    refined_labels = list()
    ...
    return (refined_preds==refined_labels).mean()
...
def compute_metrics(task_name, preds, labels,guid1_batch,guid2_batch):
...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = BertForSequenceClassification.from_pretrained(args.output_dir, num_labels=num_labels)  #testing
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case) #testing
    model.to(device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_features = convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer,'no label' )
        ...
        output_eval_file = os.path.join(args.output_dir, "test_logits.p") #testing
