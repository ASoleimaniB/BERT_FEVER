...
class InputFeatures_train(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, verification_label_id,guid):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_id = label_id
...
class FeverProcessor(DataProcessor):
    ...
    def get_train_pos_examples(self, data_dir):
        ...
        return self._create_examples_train(
            self._read_tsv(os.path.join(data_dir, "train_sentences_pos.tsv")), "train")

    def get_train_neg_examples(self, data_dir):
        """See base class."""
        return self._create_examples_train(
            self._read_tsv(os.path.join(data_dir, "train_sentences_neg_32.tsv")), "train")
    ...
    def _create_examples_train(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample_train(guid=guid, text_a=text_a, text_b=text_b, label=label,verification_label=verification_label))
        return examples
...
def convert_examples_to_features_train(examples, label_list,label_verification_list, max_seq_length,
                                 tokenizer, output_mode):
...
def main():
    ...
    if args.losstype == 'hinge_loss' or args.losstype == 'hinge_loss_concat' or args.losstype == 'hinge_loss_mining' or args.losstype == 'pairwise_independent' or args.losstype == 'pairwise_dependent':
        num_labels=1
    if args.losstype =='cross_entropy_pairwise_independent':
        num_labels = 3
    ...
    if args.do_train:
        ...
        # train_examples_pos=train_examples_pos[0:1000] #debugging
    ...
    if args.do_train:
        train_features = convert_examples_to_features_train(
            train_examples_pos, label_list,label_verification_list, args.max_seq_length,tokenizer, output_mode)

        train_features_neg = convert_examples_to_features_train(
            train_examples_neg, label_list,label_verification_list, args.max_seq_length,tokenizer, output_mode)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.losstype == 'pairwise_dependent':
                    input_ids, input_mask, segment_ids, label_ids,guid_pos = batch
                else:
                    input_ids, input_mask, segment_ids, label_ids = batch

                # define a new function to compute loss values for both output_modes
                if args.losstype=='cross_entropy' or args.losstype=='hinge_loss' or args.losstype=='cross_entropy_mining' or args.losstype=='hinge_loss_mining'\
                        or args.losstype == 'pairwise_independent' or args.losstype =='cross_entropy_pairwise_independent':
                    ...
                    with torch.no_grad():
                        logits = model(input_ids_cat, segment_ids_cat, input_mask_cat, labels=None)
                    # logits = model(input_ids_cat, segment_ids_cat, input_mask_cat, labels=None)

                elif args.losstype == 'pairwise_dependent':

                    # independet is not correct
                    input_ids_cat=torch.cat([input_ids, input_ids_neg],dim=0)
                    segment_ids_cat=torch.cat([segment_ids, segment_ids_neg],dim=0)
                    input_mask_cat=torch.cat([input_mask,input_mask_neg],dim=0)
                    # label_ids_cat=torch.cat([label_ids.view(-1), label_ids_neg.view(-1)], dim = 0)
                ...
                if output_mode == "classification":
                    ...
                    if args.losstype == 'cross_entropy':
                        loss = loss_fct(logits.view(-1, num_labels),label_ids_cat)
                        ...
                        loss = loss_fct(logits.view(-1, num_labels),label_ids_cat)
                        Neg_index = label_ids_cat.view(-1) == 1
                        Pos_index = label_ids_cat.view(-1) == 0
                        Pos_numbers = Pos_index.sum()

                        TOP_K, Hard_Neg_index = loss[Neg_index].topk(Pos_numbers)
                        # loss = torch.cat([loss[Pos_index], TOP_K])    #uncomment if dont want to use no-grad then grad

                        #comment if dont want to use no-grad then grad
                        IDS = torch.cat([torch.tensor(range(0,Pos_numbers),device=device), Pos_numbers+Hard_Neg_index], dim=0)
                        logits = model(input_ids_cat[IDS, :], segment_ids_cat[IDS, :], input_mask_cat[IDS, :], labels=None)
                        loss_fct = CrossEntropyLoss()
                        loss = loss_fct(logits.view(-1, num_labels), label_ids_cat[IDS])

                    elif args.losstype == 'hinge_loss':
                        label_ids_cat[label_ids_cat == 1] = -1
                        label_ids_cat[label_ids_cat == 0] = 1
                        loss = torch.max(torch.zeros([label_ids_cat.shape[0],1],device=device),(1- ((label_ids_cat.float()[:,None])*logits) ))
                        loss = loss.mean()

                    elif args.losstype == 'hinge_loss_mining':
                        label_ids_cat[label_ids_cat == 1] = -1
                        label_ids_cat[label_ids_cat == 0] = 1
                        loss = torch.max(torch.zeros([label_ids_cat.shape[0], 1], device=device),
                                         (1 - ((label_ids_cat.float()[:, None]) * logits)))


                        loss=torch.transpose(loss,0,1).squeeze(0)
                        # loss = loss_fct(logits.view(-1, num_labels),label_ids_cat)
                        Neg_index = label_ids_cat.view(-1) == -1
                        # Neg_numbers = Neg_index.sum()
                        Pos_index = label_ids_cat.view(-1) == 1
                        Pos_numbers = Pos_index.sum()

                        TOP_K, _ = loss[Neg_index].topk(Pos_numbers)

                        loss = torch.cat([loss[Pos_index], TOP_K]).mean()

                    elif args.losstype == 'hinge_loss_concat':
                        label_ids[label_ids == 1] = -1
                        label_ids[label_ids == 0] = 1
                        loss = torch.max(torch.zeros([label_ids.shape[0],1],device=device),(1- ((label_ids.float()[:,None])*logits) ))
                        loss = loss.mean()

                    elif args.losstype == 'pairwise_independent':
                    ...
                    elif args.losstype =='cross_entropy_pairwise_independent':
                        POS = logits[0 :int(args.train_batch_size / 2),0]
                        NEG = logits[int(args.train_batch_size / 2) : 2*int(args.train_batch_size / 2),0]
                        loss = torch.max(torch.zeros([int(args.train_batch_size / 2),1],device=device),(1 + NEG - POS ))
                        loss = loss.mean()
                        loss = loss + loss_fct(logits[:,1:].view(-1, 2),label_ids_cat)
                    else:
